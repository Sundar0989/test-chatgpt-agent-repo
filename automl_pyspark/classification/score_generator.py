...
Class responsible for generating production-ready scoring scripts for trained models.
This class creates standalone Python scripts that can be used to score new data.
"""

import os
from typing import Dict, List, Any

# Make PySpark import optional for testing
try:
    from pyspark.sql import SparkSession
    PYSPARK_AVAILABLE = True
except ImportError:
    PYSPARK_AVAILABLE = False
    # Create a dummy SparkSession class for testing
    class SparkSession:
        pass

class ScoreGenerator:
    """
    Score generator class that creates production-ready scoring scripts.
    
    This class generates standalone Python scripts for each model type that can be used
    to score new data without requiring the full AutoML pipeline.
    """
    
    def __init__(self, output_dir: str, user_id: str = "automl_user", model_id: str = "automl_model", model_literal: str = "automl_model"):
        """
        Initialize the score generator.
        
        Args:
            output_dir: Directory to save scoring scripts
            user_id: User identifier (default: "automl_user")
            model_id: Model identifier (default: "automl_model")
            model_literal: Model literal/tag (default: "automl_model")
        """
    ...
    def generate_scoring_code(self, config: Dict[str, Any], feature_vars: List[str],
        ...
        """
        print("Generating scoring scripts...")
        
        # Generate scripts for all model types
        model_types = ['logistic', 'random_forest', 'gradient_boosting', 'decision_tree', 'neural_network', 'xgboost', 'lightgbm']
        
        for model_type in model_types:
            if config.get(f'run_{model_type}', False):
                # Skip gradient boosting for multi-class problems (same logic as in model building)
                if model_type == 'gradient_boosting' and is_multiclass:
                    print(f"Skipping scoring script for {model_type} - not trained for multi-class problems")
                    continue
        ...
        print(f"Scoring scripts generated in {self.output_dir}")
    ...
    def generate_scoring_code_from_model_path(self, model_path: str, best_model_type: str):
        ...
        print(f"Scoring script generated in {self.output_dir}")
    ...
    def _generate_model_scoring_script(self, model_type: str, config: Dict[str, Any],
        ...
        # Save the script
        script_filename = f"{model_type}_model_scoring.py"
...
This script provides scoring functionality for the {model_type} model.
...
# Model variables - These will be loaded from model_info.pkl
...
class {class_name}:
    """
    {display_name} Model Scorer class for making predictions on new data.
    """
    ...
    def load_model(self):
        ...
        try:
            ...
            model_info_path = os.path.join(home_path, 'model_info.pkl')
            model_info = joblib.load(model_info_path)
            ...
            self.loaded = True
            print("{display_name} model loaded successfully.")
    ...
    def _load_model_by_type(self, model_type: str, model_path: str):
        ...
        if model_type == 'xgboost':
            # For XGBoost, load as standalone SparkXGBClassifier model
            try:
                from xgboost.spark import SparkXGBClassifierModel
                model = SparkXGBClassifierModel.load(model_path)
                print(f"Loaded XGBoost model as SparkXGBClassifierModel from {{model_path}}")
                return model
            except Exception as e:
            ...
            try:
                ...
                model = LightGBMClassificationModel.load(model_path)
            ...
            model_classes = {{
                ...
                'gradient_boosting': GBTClassificationModel,
                'decision_tree': DecisionTreeClassificationModel,
                'neural_network': MultilayerPerceptronClassificationModel
            }}
    ...
    def get_prediction_probabilities(self, predictions):
        """
        Extract prediction probabilities from predictions DataFrame.
        ...
        return predictions.select(
            col('prediction'),
            extract_prob('probability').alias('probability_positive')
        )


def main():
    """Main function for {model_type} model scoring."""
    # Create scorer
    scorer = {class_name}()
    ...
    # Get probabilities
    probabilities = scorer.get_prediction_probabilities(predictions)
...
class {class_name}:
    ...
    def load_model(self):
        ...
        try:
            ...
            print("Main pipeline model loaded successfully")
            
            # Load {model_type} model
            {model_type}_model_path = os.path.join(model_path, '{model_type}_model')
            self.model = self._load_model_by_type('{model_type}', {model_type}_model_path)
    ...
    def score(self, data):
        ...
        # Apply main pipeline (vectorization)
        pipeline_features = self.pipeline_model.transform(preprocessed_data)
        
        # Make predictions
        predictions = self.model.transform(pipeline_features)
...
def main():
    ...
    # Get Spark session
    spark = scorer.get_spark_session()
    ...
    print("Scorer initialized successfully. Use the score() method to make predictions.")
